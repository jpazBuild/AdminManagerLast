...
//         // Saca IDs y fechas, ajusta campos si lo deseas
//         const { testCaseId, createdAt, ...cloneData } = test;
//         // Si tienes `jsonSteps` y quieres asegurar, pásalo así:
...
//             return;
//         }
//         console.log("Parsed JSON for cloning:", parsed);

//         // setIsCloning(true);
//         // try {
//         //     const url = `${process.env.URL_API_INTEGRATION?.replace(/\/+$/, "")}/createAutomationFlow`;
...
//         //         }
//         //     });
//         //     toast.success("Test case clonado exitosamente");
//         //     setCloneModalOpen(false);
//         //     onRefreshAfterUpdateOrDelete();
//         // } catch (err: any) {
//         //     setJsonError(err?.response?.data?.error || err.message || "Error clonando test case");
//         // } finally {
//         //     setIsCloning(false);
//         // }
//     };
...
//                 <DialogContent className="max-w-2xl max-h-[60vh] bg-white p-6 rounded-lg shadow-lg">
//                     <DialogHeader>
//                         <DialogTitle className="text-primary">Cloning Test Case</DialogTitle>
//                     </DialogHeader>
//                     <div>
//                         <textarea
...
const SortableTestCaseItem: React.FC<Props> = ({
    ...
    // Hook para preservar scroll - incluye todas las dependencias que causan re-renders
    ...
    // Función para manejar toggle con preservación de scroll
    const handleToggleSelect = () => {
        toggleSelect(test.testCaseId ?? '');
    };

    // Función para manejar apertura/cierre de acordeón con preservación de scroll
            ...
            <AccordionItem value={test.testCaseId ?? ''} className="rounded-lg">
                ...
                <AccordionContent className="p-2 w-full">
                    ...
                    {viewMode === 'data' && (
                        <div 
                            ref={scrollRef}
                            className="flex flex-col gap-4 max-h-[60vh] overflow-y-auto px-1 mt-4 p-2"
                        >
                            {testFields.map((field, idx) => (
                                <div key={`${field}-${idx}`} className="flex flex-col gap-4 px-1 break-words">
                                    <Label className="w-full text-primary/90 break-words max-w-[480px] truncate">{field}</Label>
                    ...
                    {viewMode === 'steps' && (
                        <div 
                            ref={scrollRef}
                            className="flex flex-col gap-4 max-h-[100vh] overflow-y-auto px-1"
                        >
                            <div className="self-end mt-4 mb-3 flex gap-1 items-center border-2 border-primary/60 rounded-md p-1">
                                <span>Copy All steps</span>
                                <CopyToClipboard text={JSON.stringify(steps)} />
