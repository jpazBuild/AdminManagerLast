...
import { Check, CircleAlert, PlusIcon, Settings, X } from "lucide-react";
...
const EnvironmentsPage = () => {
  ...
  const toggleAll = () =>
    setRows(prev => prev.map(r => (filtered.some(f => f.id === r.id) ? { ...r, enabled: !allEnabled } : r)));
  ...
  const onSave = async (opts?: { nameOverride?: string }) => {
    ...
    try {
      ...
      if (opts?.nameOverride) {
        setSelectedEnvironment((prev: any) => (prev ? { ...prev, name: effectiveName } : prev));
      }
  ...
  const renameEnvironment = async (newName: string) => {
    ...
    try {
      ...
      setSelectedEnvironment((prev: any) => (prev ? { ...prev, name: newName.trim() } : prev));
  ...
  return (
    <DashboardHeader pageType="api" callback={setMobileSidebarOpen} onDarkModeChange={setDarkMode}>
      <div
        className={`flex gap-2 w-full h-full overflow-hidden ${
        ...
        >
          <div
            className={`flex-shrink-0 p-4 border-b ${
              darkMode ? "bg-gray-900 border-gray-700" : "bg-white border-primary/10"
            }`}
          >
          ...
          {environments.length > 0 ? (
            <div className="flex overflow-y-auto w-full flex-col gap-1 p-2">
              {environments.map((env) => (
                <div
                  key={env.id}
                  className={`p-3 cursor-pointer rounded-lg flex items-center justify-between ${
                    darkMode
                      ? `hover:bg-gray-800 bg-gray-800 ${selectedEnvironment?.id === env.id ? "bg-gray-800" : ""}`
                      : `hover:bg-primary/5 ${selectedEnvironment?.id === env.id ? "bg-primary/10" : ""}`
                  }`}
                  onClick={() => {
                ...
                >
                  <h3 className={`font-medium ${darkMode ? "text-gray-100" : "text-primary/80"}`}>{env.name}</h3>
                  {selectedEnvironment?.id === env.id && (<Check className={`w-5 h-5 ${darkMode ? "text-white/90" : "text-primary"}`} />)}
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="flex h-full w-full flex-col overflow-hidden justify-center self-center items-center">
          ...
          {!selectedEnvironment && !createView && (
            <div className="flex w-full h-full items-center justify-center p-4 flex-col gap-2">
              <Settings className={`${darkMode ? "text-primary" : "text-[#3956E8]"} w-20 h-20`} />
              <p className={`text-[24px] font-semibold tracking-wider ${darkMode ? "text-gray-100" : "text-primary/85"}`}>
                Select an environment
              </p>
              <p className={`text-[14px] ${darkMode ? "text-gray-400" : "text-gray-500"}`}>You could see and edit the data</p>
            </div>
          )}
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <div
            className={`relative z-10 w-full max-w-md rounded-xl p-6 shadow-xl flex flex-col ${
              darkMode ? "bg-gray-900 border border-gray-700" : "bg-white"
            }`}
          >
            <div className="flex items-center mb-2 w-full">
              <div className="flex flex-col items-center gap-2 w-full justify-center">
                <CircleAlert className={`w-8 h-8 ${darkMode ? "text-primary" : "text-primary/90"}`} />
                <h3 className={`text-base text-center font-semibold ${darkMode ? "text-gray-100" : "text-primary/90"}`}>
            ...
            <div className="flex items-center w-full gap-2">
              <button
                className={`w-1/2 px-4 py-2 rounded-lg text-sm ${
                  darkMode ? "border border-gray-700 hover:bg-gray-800" : "border hover:bg-gray-50"
                }`}
                onClick={() => setShowResetConfirm(false)}
              >
                Cancel
              ...
              <button
                className={`w-1/2 px-4 py-2 rounded-lg text-sm text-white ${
      ...
      {toastMsg && (
        <div className="fixed lg:w-1/2 bottom-4 left-1/2 -translate-x-1/2 z-40">
          <div
            className={`rounded-lg px-4 py-2 shadow flex justify-between items-center ${
              darkMode
          ...
          >
            {toastMsg}
            <button
              className={darkMode ? "ml-2 text-emerald-300" : "ml-2 text-green-500"}
      ...
      {toastError && (
        <div className="fixed lg:w-1/2 bottom-4 left-1/2 -translate-x-1/2 z-40">
          ...
          >
            {toastError}
            <button
              className={darkMode ? "ml-2 text-red-300" : "ml-2 text-red-500"}
