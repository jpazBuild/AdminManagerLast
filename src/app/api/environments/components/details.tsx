        ...
        <div className="flex flex-col h-full">
          {isEditingName ? (
            <TextInputWithClearButton
              ...
              isSearch={false}
              className={`text-xl font-bold ${darkMode ? "text-gray-100" : "text-primary/85"}`}
              isDarkMode={darkMode}
            />
          ) : (
            <h2
              className={`text-2xl font-bold ${darkMode ? "text-gray-100" : "text-primary/85"}`}
              onDoubleClick={() => {
                if (environment) setIsEditingName(true);
              }}
          ...
          <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Edit variables and values</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          {isDirty && (
            <button
              ...
              className={`cursor-pointer text-sm px-3 py-2 rounded-full hover:opacity-80 ${darkMode ? "text-white bg-gray-800" : "bg-gray-200 text-primary"}`}
            >
              Reset changes
            </button>
          )}
          <button
            onClick={onSaveAll}
            disabled={!canSave}
            className={`px-5 py-2 rounded-lg text-white transition ${darkMode ? "bg-primary-blue/90":"bg-primary"} ${
              !canSave ? "cursor-not-allowed" : ""
            }`}
          >
            {saving ? "Saving..." : "Save"}
          </button>
          <MoreMenu darkMode={darkMode} disabled={saving} onDelete={onDelete} />
        </div>
      </div>

      <div className="flex flex-col w-full h-full overflow-y-auto px-6">
        <div
          className={`grid grid-cols-[28px_1fr_1fr_40px] gap-3 py-2 text-xs ${
        ...
        <div className="h-full flex flex-col gap-2 overflow-y-auto overflow-x-hidden">
          {rows.length > 0 ? (
            rows.map((r) => (
              <div
                key={r.id}
                className={`grid grid-cols-[28px_1fr_1fr_40px] gap-2 items-center rounded-lg ${
                  darkMode ? "" : "hover:bg-primary/5"
                }`}
              >
                <div className="flex items-center">
                  <input
                ...
                <button
                  className={`p-2 rounded ${
                    darkMode ? "text-white hover:bg-gray-800" : "text-primary/60 hover:bg-gray-100"
                  }`}
            ...
            <div className={`px-3 py-8 text-center text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
          ...
          <div className="mt-2 flex justify-between items-center">
            <button
              className={`px-3 py-2 rounded-lg text-sm flex gap-1 items-center ${
                darkMode
                  ? "bg-white/50 text-primary hover:bg-white/60"
