...
const mapEnvToRows = (env?: Record<string, string> | null): KV[] =>
  env && Object.keys(env).length
    ? Object.entries(env).map(([k, v]) => ({ id: newId(), enabled: true, key: k, value: String(v ?? "") }))
    : [{ id: newId(), enabled: true, key: "Default", value: "Default" }];

const rowsToEnv = (rows: KV[]): Record<string, string> => {
...
const EnvEditor: React.FC<Props> = ({
  initialEnv,
  initialName,
  onCancel,
  onSave,
  ...
  useEffect(() => {
    ...
    const nextFromProp = initialEnv ?? {};
    const nextKey = JSON.stringify(nextFromProp);

    if (prevInitEnvKeyRef.current === null) {
      prevInitEnvKeyRef.current = nextKey;
      return;
    }

    if (prevInitEnvKeyRef.current !== nextKey && !deepEqual(currentFromRows, nextFromProp)) {
  ...
  useEffect(() => {
    ...
    const next = { name: payload.name, env: payload.env };
    if (!lastDraftRef.current || !deepEqual(lastDraftRef.current, next)) {
