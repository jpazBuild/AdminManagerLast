...
const FlowCanvas: React.FC<Props> = ({
  ...
  const handleSaveTest = async () => {
    ...
    const iterationDataResponse: any = await (async () => {
  ...
  const tagsOptions = useMemo(() => (tags || []).map((t) => ({ label: t.name, value: t.id })), [tags]);

  const outlineBtn = darkMode
    ? "border-gray-500 text-gray-100 hover:bg-gray-800"
    : "border-primary-blue text-primary/80 hover:bg-primary/5";
  const primaryBtn = darkMode
    ? "bg-primary-blue border-primary-blue text-white hover:bg-primary-blue/90"
    : "bg-primary-blue border-primary-blue text-white";
  const canvasText = darkMode ? "text-gray-100" : "text-primary/80";
  const connectorStroke = darkMode ? "rgba(255,255,255,0.35)" : "rgba(57,86,232,0.45)";

  return (
    <div className="w-full flex flex-col gap-4">
      <div ref={containerRef} className={`relative flex items-start gap-6 flex-wrap ${canvasText}`}>
        {visibleFlow.map((n, i) => (
          <div key={n.id} ref={setCardRef(i)} className="relative z-30">
            <FlowCard darkMode={darkMode} node={n} onOpen={onOpenNode} onChangeUrl={onChangeUrl} onRemove={onRemoveNode} />
          </div>
        ))}
      ...
      <div className="self-end flex gap-2 items-center pb-2">
        <button
          ...
          className={`cursor-pointer font-semibold px-5 py-2 rounded-2xl shadow-md border-2 ${outlineBtn}`}
        >
          Save Flow
        </button>
        <button
          onClick={onSendFlow}
          className={`cursor-pointer font-semibold px-5 py-2 rounded-2xl shadow-md border-2 ${primaryBtn}`}
        >
          Run Flow
        </button>
      </div>
